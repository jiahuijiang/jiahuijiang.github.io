...
let sketchConfetti = function (p) {
  ...
  class Confetti {
    confettiColors = [p.color(168, 100, 253), p.color(41, 205, 255), p.color(120, 255, 68), p.color(255, 113, 141), p.color(253, 255, 106)];
    ...
    show() {
      ...
      if (p.mouseX < p.width && p.mouseX > 0 && p.mouseY < p.height && p.mouseY > 0) {
        if (p.abs(this.x - p.mouseX) < 100) {
          this.ySpeedFromWind =
            (getWindSpeed().speedY / (p.log(p.abs(this.y - p.mouseY) + 1) + 1)) *
            p.sqrt(p.abs(p.abs(this.x - p.mouseX) - 100)) *
            yWindSpeedMultiplier;
        }
        if (p.abs(this.y - p.mouseY) < 100) {
          this.xSpeedFromWind =
            (getWindSpeed().speedX / (p.log(p.abs(this.x - p.mouseX) + 1) + 1)) *
            p.sqrt(p.abs(p.abs(this.y - p.mouseY) - 100)) *
            xWindSpeedMultiplier;
        }
      }
